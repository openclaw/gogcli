...
- Docs: `docs write --append --markdown` no longer silently drops tables with `insert native table: table not found near index N`. The native-table inserter's post-write search used a ±2 code-unit window, but the Docs API's actual table StartIndex can drift further (auto-newline + placeholder paragraph combine to a several-unit shift); the search now picks the closest forward Table element with a small backward tolerance instead. The `docs create --file --markdown` path was unaffected because it uses Drive's native markdown import end-to-end. (#592)
