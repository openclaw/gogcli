...
- Drive: add `--action=resolve|reopen` to `drive comments reply` and sibling `drive comments resolve|reopen` verbs (also `docs comments reopen`) to post a reply that atomically flips the parent comment's resolved state via the Drive API's `Reply.action` field. Avoids the previous workaround of `drive comments delete` (which destroys review-thread context) for batch-resolving inline doc-review feedback. (#623)
